...
ss::future<> coordinator_stm::do_apply(const model::record_batch& batch) {
    if (batch.header().type != model::record_batch_type::raft_data) {
        co_return;
    }
    co_await batch.for_each_record_async(
      [this](const model::record& r) mutable {
          auto entry = serde::from_iobuf<translated_data_file_entry>(
            r.value().copy());
          _all_files[entry.tp.topic][entry.tp.partition].files.emplace_back(
            std::move(entry));
      });
}
...
coordinator_stm::add_translated_data_file(
  add_translated_data_files_request req) {
    ...
    auto guard = ss::defer(
      [&partition_state] { partition_state.request_in_progress = false; });
