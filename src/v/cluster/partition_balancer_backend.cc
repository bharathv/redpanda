...
ss::future<> partition_balancer_backend::do_tick() {
    ...
    _cur_term->last_tick_draining_realloc_failures = std::move(
      plan_data.draining_realloc_failures);
    if (
...
partition_balancer_overview_reply partition_balancer_backend::overview() const {
    ...
    ret.draining_realloc_failures
      = _cur_term->last_tick_draining_realloc_failures;
