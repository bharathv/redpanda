...
void producer_state_manager::do_evict_excess_producers() {
    ...
    while (it != _lru_producers.end() && can_evict_producer(*it)) {
        auto it_copy = it;
        auto& state = *it_copy;
        state.deregister_self(); // unlinks from the intrusive list
...
void producer_state_manager::register_producer(producer_state& state) {
    vlog(clusterlog.info, "Registered producer: {}", state._id);
    _lru_producers.push_back(state);
    _num_producers++;
}

void producer_state_manager::deregister_proudcer(producer_state& state) {
    state._hook.unlink();
    _num_producers--;
}
}; // namespace cluster
