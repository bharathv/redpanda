...
private:
    ...
    ss::future<tx_errc> do_commit_tx(
      tx::producer_ptr, model::tx_seq, model::timeout_clock::duration);
    ...
    // side property to it's corresponding producer_id using init_producer_id
    // request and the coordinator ensures fencing by only retaining the latest
    // epoch for a given producer_id.
    //
