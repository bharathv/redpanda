...
private:
    ...
    void
    report_draining_reallocation_failure(model::node_id, const model::ntp&);
...
void partition_balancer_planner::request_context::
  report_draining_reallocation_failure(
    model::node_id node, const model::ntp& ntp) {
    auto& ntps = _draining_realloc_failures.try_emplace(node).first->second;
              ...
              for (const auto& replica : to_move) {
                  if (part.is_original(replica)) {
                      auto result = part.move_replica(
                        ...
                        reason);
                      if (!result) {
                          ctx.report_draining_reallocation_failure(
                            replica, part.ntp());
                      }
                  }
              }
...
void partition_balancer_planner::request_context::collect_actions(
  partition_balancer_planner::plan_data& result) {
    ...
    result.draining_realloc_failures = std::move(_draining_realloc_failures);
