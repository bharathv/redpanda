...
ss::future<result<kafka_result>> rm_stm::do_idempotent_replicate(
  ...
  ssx::semaphore_units units) {
    ...
    auto synced_term = _insync_term;
    auto request = producer->enqueue_request(bid, synced_term);
    if (std::holds_alternative<errc>(request)) {
        co_return std::get<errc>(request);
    } else if (std::holds_alternative<cached_result_t>(request)) {
        co_return co_await std::move(std::get<cached_result_t>(request));
    }
    auto req_ptr = std::get<request_ptr>(request);
...
ss::future<result<kafka_result>> rm_stm::idempotent_replicate(
  ...
  ss::lw_shared_ptr<available_promise<>> enqueued) {
    ...
    auto result = co_await producer->run_func([&](ssx::semaphore_units units) {
        return do_idempotent_replicate(
          producer,
          bid,
          std::move(br),
          opts,
    ...
    if (!result) {
        ...
        if (
          _raft->is_leader()
          && (result.error() == errc::replication_error || result.error() == errc::timeout)) {
