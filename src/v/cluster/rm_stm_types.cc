...
tx_snapshot::tx_snapshot(tx_snapshot_v4 snap_v4, raft::group_id group)
  ...
  , expiration(std::move(snap_v4.expiration)) {
    for (auto& entry : snap_v4.seqs) {
        cluster::tx::producer_state_snapshot snapshot;
        ...
        for (auto& req : entry.seq_cache) {
            cluster::tx::producer_state_snapshot::finished_request request;
...
ss::future<> async_adl<tx_snapshot_v4>::to(iobuf& out, tx_snapshot_v4 snap) {
    ...
    co_await detail::async_adl_list<fvec<cluster::tx::deprecated_seq_entry>>{}
      .to(out, std::move(snap.seqs));
    co_await detail::async_adl_list<fvec<cluster::tx::tx_data_snapshot>>{}.to(
      out, std::move(snap.tx_data));
    co_await detail::async_adl_list<fvec<cluster::tx::expiration_snapshot>>{}
...
ss::future<tx_snapshot_v4> async_adl<tx_snapshot_v4>::from(iobuf_parser& in) {
    ...
    auto seqs = co_await detail::async_adl_list<
                  fvec<cluster::tx::deprecated_seq_entry>>{}
                  .from(in);
    auto tx_data
      = co_await detail::async_adl_list<fvec<cluster::tx::tx_data_snapshot>>{}
          .from(in);
    auto expiration = co_await detail::async_adl_list<
                        fvec<cluster::tx::expiration_snapshot>>{}
                        .from(in);
