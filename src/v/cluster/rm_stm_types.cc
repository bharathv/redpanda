...
tx_snapshot_v6::tx_snapshot_v6(tx_snapshot_v5 snap_v5)
  ...
  , highest_producer_id(snap_v5.highest_producer_id) {
    ...
    // todo (bharathv): there are edge cases in this code, fix
    // and add an upgrade test
...
void adl<model::tx_range>::to(iobuf& out, model::tx_range t) {
    reflection::serialize(out, t.first, t.last, t.pid);
}
