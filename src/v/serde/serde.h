...
int64_t checked_duration_cast_to_nanoseconds(
  const std::chrono::duration<R, P>& duration) {
    ...
    if (unlikely(__builtin_mul_overflow(dur, ratio_num, &mul))) {
        ...
        //
        // If you are here because your ducktape test failed with this BadLogLine
        // check, it means that you serialized a duration type that caused an
        // overflow when casting to nanoseconds. Clamp your duration to
        // nanoseconds::max().
