...
ss::future<> mux_remote_consumer::stop() noexcept {
    ...
    co_await ss::max_concurrent_for_each(
      _partitions, 5, [](auto& p) { return p.second.stop(); });
}

void mux_remote_consumer::add(const model::topic_partition& tp) {
    auto holder = _gate.hold();
    vassert(!_partitions.contains(tp), "Duplicate partition added: {}", tp);
    _partitions.emplace(tp, partition_data_queue{_partition_max_buffered});
    _pending_assignment[tp.topic].insert(tp.partition);
    _cv.signal();
}

ss::future<> mux_remote_consumer::remove(const model::topic_partition& tp) {
    auto holder = _gate.hold();
...
void mux_remote_consumer::reset(
...
ss::future<partition_data_queue::fetch_data> mux_remote_consumer::fetch(
