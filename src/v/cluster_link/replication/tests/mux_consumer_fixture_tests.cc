...
TEST_P(MuxConsumerFixture, TestProduceConsume) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        _mux_consumer->add(tp);
        _mux_consumer->reset(tp, kafka::offset{0});
    }
        ...
        fetchers.push_back(
          ss::async([this, i, &consumed, &as]() {
              ...
              while (!as.abort_requested()) {
                  auto data = _mux_consumer->fetch(tp, as).get();
...
TEST_P(MuxConsumerFixture, TestResetPartition) {
    ...
    _mux_consumer->reset(tp, kafka::offset{101});
    ...
    auto batches = std::move(_mux_consumer->fetch(tp, as).get().batches);
