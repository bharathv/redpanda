...
struct test_fixture {
    ...
    test_fixture() {
        ...
        _config.start(ss::smp::count * initial_rate_per_shard).get();
        _throttler
          .start(
            ss::sharded_parameter([this] { return _config.local().bind(); }))
          .get();
    ...
    ss::sharded<config::mock_property<size_t>> _config;
