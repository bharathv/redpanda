...
TEST_F_CORO(raft_fixture, test_recovery_after_compaction_with_gaps) {
    ...
    auto replicate =
      [&](
        ...
        consistency_level cl = consistency_level::leader_ack) -> ss::future<> {
        auto batches = make_batches_of_type(batch_type);
        auto r = co_await retry_with_leader(
          model::timeout_clock::now() + 30s,
          [batches = std::move(batches),
           cl](raft_node_instance& leader_node) mutable {
              return leader_node.raft()
                ->replicate(std::move(batches), replicate_options(cl))
    ...
    // which is needed to excercise this code path.
    ...
    storage::housekeeping_config hk_cfg(
      ...
      std::nullopt,
      model::offset::max(),
      stm_mgr ? stm_mgr->max_tombstone_remove_offset() : model::offset::max(),
      stm_mgr ? stm_mgr->max_tx_end_remove_offset() : model::offset::max(),
      0ms,
      0ms,
      0ms,
      as);
    ...
    // sent data. In typical cluster scenarions this could be from a leadership
    ...
    node(leader_id).set_reply_interceptor(
      [blocked_follower_id, grp, interceptor_active, base_committed_offset](
        reply_variant reply,
        model::node_id source_node) -> ss::future<reply_variant> {
          ...
          *interceptor_active = false;
          ...
          co_return reply_variant{std::move(modified)};
      });
