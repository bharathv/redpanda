...
operator<<(std::ostream& o, const replication_monitor::waiter& entry) {
    fmt::print(o, "{{type: {}, append result: {}}}", entry.type, entry.result);
    return o;
}
...
ss::future<> replication_monitor::do_notify_replicated() {
    while (!_gate.is_closed()) {
        ...
        while (it != _waiters.end()
            ...
            auto replicated_term = _raft->get_term(entry.result.last_offset);
            if (
              ...
              && replicated_term == entry.result.last_term
              && majority_replicated_offset >= entry.result.last_offset) {
...
ss::future<> replication_monitor::do_notify_committed() {
    while (!_gate.is_closed()) {
        ...
        while (
          it != _waiters.end()
          && (it->first <= committed_offset || it->second.result.last_term < committed_offset_term)) {
            auto& entry = it->second;
            auto appended_term = _raft->get_term(entry.result.last_offset);
            ...
            auto truncated = [&] {
                return committed_offset_term > entry.result.last_term
                       && appended_term != entry.result.last_term;
            };
            if (
              appended_term == entry.result.last_term
              && committed_offset >= entry.result.last_offset) {
                // committed
                entry.done.set_value(errc::success);
                it = _waiters.erase(it);
            } else if (truncated()) {
