...
ss::rwlock& fetcher::shared_state_lock() { return _parent->_fetchers_lock; }
...
ss::future<chunked_vector<fetcher::partitions_to_process>>
fetcher::collect_partitions() {
    chunked_vector<partitions_to_process> ret;
    ret.reserve(_partitions.size());
    ...
    for (auto& [topic, partitions] : _partitions) {
        ...
        if (!to_process.empty()) {
            ret.push_back(std::move(to_process));
        }
    }
    ...
    for (auto& topic_partitions : to_process) {
        ...
        co_await ssx::async_for_each_counter(
          ...
          [this, &f_topic](const partition_fetch_state& f_state) {
              ...
              f_partition.fetch_offset = *f_state.fetch_offset;
...
void fetcher::maybe_update_fetch_offset(
  const model::topic& topic,
  model::partition_id partition_id,
  model::offset last_received) {
    ...
    vlog(
      ...
      topic,
      partition_id,
      model::next_offset(last_received));
    p_it->second.fetch_offset = model::next_offset(last_received);
}
...
ss::future<> fetcher::do_fetch() {
    ...
    try {
        ...
        // this is the place where the loop will be stopped when the parent
        // consumer updates fetchers state.
        auto lock = co_await lock_state();
        /**
         ...
         * request.
         */
        auto partitions = co_await collect_partitions();
        ...
        auto req = co_await make_fetch_request(std::move(partitions));
        ...
        if (fetch_result.has_error()) {
            if (is_retriable_error(fetch_result.error())) {
                ...
                auto f = queue().push_error(fetch_result.error());
                lock.return_all();
                co_await std::move(f);
                co_return;
            }
        ...
        if (!fetch_result_value.topics.empty()) {
            ...
            // we do not need to hold the lock after we have pushed the data
            // to the queue, data order is now locked, we can release the lock
            // and wait for the space in queue
            lock.return_all();
            co_await std::move(f);
        }
...
ss::future<fetcher::state_lock_holder> fetcher::lock_state() {
    /**
     * VERY IMPORTANT: we need to hold the read lock on the
     * _parent->_fetchers_lock before we acquire the state lock. Otherwise we
     * may experience a deadlock as parent is holding the write lock while
     * modifying the fetcher state and it is acquired before the state lock.
     */
    auto shared_units = co_await _parent->_fetchers_lock.hold_read_lock(_as);
    auto state_units = co_await _state_lock.get_units();
    co_return state_lock_holder(
      std::move(shared_units), std::move(state_units));
}

ss::future<kafka_result<fetcher::fetch_response_content>>
fetcher::process_fetch_response(fetch_response resp) {
    ...
    for (auto& topic_response : resp.data.responses) {
        ...
        for (auto& part_response : topic_response.partitions) {
            ...
            if (part_response.error_code != kafka::error_code::none) {
                ...
                part_data.high_watermark = part_response.high_watermark;
                ...
                maybe_update_fetch_offset(
                  topic_data.topic,
                  part_data.partition_id,
                  part_data.data.back().last_offset());
            }
  ...
  chunked_vector<partitions_to_process>& partitions) {
    ...
    for (auto& topic : offsets.value()) {
        for (auto& partition_offset : topic.offsets) {
            ...
            _partitions[topic.topic][partition_offset.partition_id].fetch_offset
              = partition_offset.offset;
        }
    }
...
ss::future<> fetcher::assign_partition(
  model::topic_partition_view tp, std::optional<model::offset> offset) {
    auto lock = _state_lock.get_units();
    vlog(
      logger().debug,
