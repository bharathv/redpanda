...
ss::future<> direct_consumer::update_fetchers(
  chunked_hash_map<model::topic, chunked_vector<model::partition_id>>
    removals) {
    auto holder = _gate.hold();
    // Stop fetch loops by acquiring a write lock
    auto u = co_await _fetchers_lock.hold_write_lock();
    for (auto& [topic, partitions] : removals) {
        for (auto& p : partitions) {
            for (auto& [id, fetcher] : _broker_fetchers) {
                // Unassign partitions from all fetchers
                co_await fetcher->unassign_partition(
                  model::topic_partition_view(topic, p));
            }
        }
    }
...
direct_consumer::unassign_topics(chunked_vector<model::topic> topics) {
    ...
    for (const auto& topic : topics) {
        ...
        for (const auto& [p_id, _] : state_it->second) {
            removals[topic].push_back(p_id);
        }
  ...
  chunked_vector<model::topic_partition> partitions) {
    ...
    for (const auto& tp : partitions) {
        ...
        if (p_it != state_it->second.end()) {
            removals[tp.topic].push_back(tp.partition);
