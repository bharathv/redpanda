	...
	for _, group := range offsetFetch.Groups {
		for _, topic := range group.Topics {
			for _, partition := range topic.Partitions {
				...
				if state != nil && state.groupAssigned && fetchedOffset >= 0 && fetchedOffset <= state.lastOffsetConsumed {
					isDuplicate = true
		...
		go func(fiberId int) {
			for {
				...
				if err != nil {
					...
					// When a consumer group member restarts, it will rejoin and get
					// partitions assigned via OnPartitionsAssigned, which will properly
					// reset the monotonicity state based on the committed offsets.
					// No need to reset state here - let the assignment hook handle it.
...
func (grw *GroupReadWorker) pollAndProcessRecords(
	...
	// Defer ensures it's called even in error cases.
