...
class PartitionForceReconfigurationTest(EndToEndTest, PartitionMovementMixin):
    ...
    def _force_reconfiguration(self, alive_replicas, killed_replicas=[]):
        replicas = [
            dict(node_id=replica.node_id, core=replica.core)
            for replica in killed_replicas + alive_replicas
        ]
        ...
        if not killed_replicas:
            self.redpanda._admin.await_stable_leader(
                topic=self.topic,
                replication=len(alive_replicas),
                hosts=self._alive_nodes())
