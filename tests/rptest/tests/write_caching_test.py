...
class WriteCachingPropertiesTest(RedpandaTest):
    ...
    def validate_topic_configs(self, write_caching: WriteCachingMode,
        ...
        assert configs[self.WRITE_CACHING_PROP][0] == str(
            write_caching), configs
        assert configs[self.FLUSH_MS_PROP][0] == str(flush_ms), configs
        assert configs[self.FLUSH_BYTES_PROP][0] == str(flush_bytes), configs
        ...
        for replica in replicas:
            ...
            assert raft_state[self.WRITE_CACHING_DEBUG_KEY] == bool(
                write_caching), raft_state
            assert raft_state[self.FLUSH_MS_DEBUG_KEY] == flush_ms, raft_state
            assert raft_state[
                self.FLUSH_BYTES_DEBUG_KEY] == flush_bytes, raft_state
    ...
    def test_properties(self):
        ...
        #TODO: Fix flakyness due to delay in config propagation
